...
var getBaseOptions = function (cb) {
  var options = {}
  var git_commit = process.env.CLIOSTATS_GIT_COMMIT
  var git_branch = process.env.CLIOSTATS_GIT_BRANCH
  ...
  options.run_at = process.env.CLIOSTATS_RUN_AT || JSON.stringify(new Date()).slice(1, -1)
  if (process.env.CLIOSTATS_SERVICE_NAME) {
    options.service_name = process.env.CLIOSTATS_SERVICE_NAME
  }
  if (process.env.CLIOSTATS_SERVICE_JOB_ID) {
    options.service_job_id = process.env.CLIOSTATS_SERVICE_JOB_ID
  }
  ...
  if (process.env.CLIOSTATS_PARALLEL) {
    options.parallel = true
  }

  // try to get the repo token as an environment variable
  if (process.env.CLIOSTATS_REPO_TOKEN) {
    options.repo_token = process.env.CLIOSTATS_REPO_TOKEN
  } else {
    // try to get the repo token from a .coveralls.yml file
    var yml = path.join(process.cwd(), '.cliostats.yml')
    try {
