...
var testGitHash = function (sut, done) {
  process.env.CLIOSTATS_GIT_COMMIT = 'e3e3e3e3e3e3e3e3e'
  sut(function (err, options) {
...
var testRepoTokenDetection = function (sut, done) {
  ...
  var file = path.join(process.cwd(), '.cliostats.yml'), token, service_name, synthetic = false
  if (fs.exists(file)) {
